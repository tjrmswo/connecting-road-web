...
> FSD 레이어 구조, thin shell 패턴, 실제 리팩토링 중 발견한 버그(Rules of Hooks 위반, `ensureQueryData` 캐시 null 이슈)는 [노션 문서](https://ahead-bug-3de.notion.site/FSD-3a0b438e5ecb8115b21bff806280a635?source=copy_link)에 정리했습니다.
...
// login/page.tsx
import { ButtonList } from '@/features/auth'; // 1차 배럴 호출

// ButtonList/index.tsx 내부
import { KakaoButton, GoogleButton, NaverButton } from '@/features/auth'; // 2차 배럴 호출
...
> 레이어별 캡처 전략의 상세 근거, 정상 미인증과 진짜 장애를 구분한 방식, 인스트루먼테이션 훅 코드는 [노션 문서](https://ahead-bug-3de.notion.site/PostHog-39cb438e5ecb8119b681f2aa40d175a0?source=copy_link)에 정리했습니다.
...
> 통합 에러 라우팅 맵, 사전 검증·사후 처리 코드, `returnTo` 복귀 메커니즘은 [노션 문서](https://ahead-bug-3de.notion.site/3a0b438e5ecb8105a7c6db3cec62b652?source=copy_link)에 정리했습니다.
